...
export const admin = {
  ...
  email: 'admin@cravio.test',
  role: 'admin',
};

export class MemoryUsers {
  /** @param {any[]} [seed] */
  constructor(seed = [owner, admin]) {
    this.rows = seed.map((item) => ({ ...item }));
  }
  async findByClerkId(id) {
    return this.rows.find((item) => item.clerk_user_id === id) || null;
  }
  async findById(id) {
    return this.rows.find((item) => item.id === id) || null;
  }
  async list({ limit, offset }) {
    return { items: this.rows.slice(offset, offset + limit), total: this.rows.length };
  }
  ...
  async disableByClerkId(id) {
    const row = await this.findByClerkId(id);
...
export class MemoryAdminRepository {
  ...
  async countActiveAdmins() {
    return this.users.rows.filter((x) => x.role === 'admin' && x.status === 'active').length;
  }
  ...
  async changeUserRole(adminId, id, role) {
    const row = await this.users.findById(id);
    if (!row) return null;
    const old = row.role;
    row.role = role;
  ...
  async getOwner(id) {
    const row = await this.users.findById(id);
    return row?.role === 'owner' ? { ...row, restaurant: null } : null;
  }
  async listOwners({ page, limit, status, search }) {
    const filtered = this.users.rows.filter(
      (x) =>
        x.role === 'owner' &&
        (!status || x.status === status) &&
        (!search ||
          `${x.first_name} ${x.last_name} ${x.email}`.toLowerCase().includes(search.toLowerCase())),
    );
    return { items: filtered.slice((page - 1) * limit, page * limit), total: filtered.length };
  }
  async metrics() {
    return {
      total_owners: this.users.rows.filter((x) => x.role === 'owner').length,
      active_owners: this.users.rows.filter((x) => x.role === 'owner' && x.status === 'active')
        .length,
      total_restaurants: 0,
  ...
  return {
    ...
    app: createApp({
      ...
      authResolver: () => {
        ...
        return authUserId
          ? { isAuthenticated: true, userId: authUserId }
