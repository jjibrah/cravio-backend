...
test('category creation rejects unauthenticated, suspended, disabled, admin, and blank-name requests', async () => {
  assert.equal(
    (
      await request(menuApp({ authUserId: null }).app)
  ...
  for (const status of ['suspended', 'disabled']) {
    ...
    assert.equal(
      (await request(menuApp({ users }).app).post('/api/menu/categories').send(validCategory))
        .status,
      403,
    );
  }
  assert.equal(
    (
      await request(menuApp({ authUserId: 'clerk_admin' }).app)
        .post('/api/menu/categories')
        .send(validCategory)
    ).status,
    403,
  ...
  assert.equal(
    (await request(menuApp().app).post('/api/menu/categories').send({ name: ' ' })).status,
    400,
  );
});
test('categories list in display order and can update safe fields', async () => {
  ...
  const b = (await request(setup.app).post('/api/menu/categories').send({ name: 'Drinks' })).body
    .data;
  await request(setup.app)
    .post('/api/menu/categories/reorder')
    .send({ category_ids: [b.id, a.id] });
  ...
  const updated = await request(setup.app)
    .patch(`/api/menu/categories/${a.id}`)
    .send({ description: 'Updated', is_active: false });
  assert.equal(updated.body.data.is_active, false);
  assert.equal(
    (
      await request(setup.app)
        .patch(`/api/menu/categories/${a.id}`)
        .send({ restaurant_id: restaurantB.id })
    ).status,
    400,
  );
});
...
test('empty category deletes but a category containing items returns conflict', async () => {
  ...
  assert.equal(response.body.error.code, 'CATEGORY_HAS_ITEMS');
});
test('duplicate and incomplete category reorder payloads are rejected', async () => {
  const setup = menuApp();
  const a = await setup.menus.createCategory(restaurantA.id, { name: 'A' });
  const b = await setup.menus.createCategory(restaurantA.id, { name: 'B' });
  assert.equal(
    (
      await request(setup.app)
        .post('/api/menu/categories/reorder')
