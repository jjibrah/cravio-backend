export class PublicMenuRepository {
  ...
  async findContext(restaurantId, tableId) {
    const { rows } = await this.db.query(
      `SELECT r.id AS restaurant_id,r.name AS restaurant_name,r.slug,r.description,r.logo_url,r.cover_image_url,r.currency,
      ...
      WHERE r.id=$1 AND t.id=$2 AND r.status='active' AND r.is_published=TRUE AND t.is_active=TRUE`,
  ...
  async findMenuRows(restaurantId, availableOnly) {
    const { rows } = await this.db.query(
      `SELECT c.id AS category_id,c.name AS category_name,c.description AS category_description,c.display_order AS category_order,
      ...
      ORDER BY c.display_order,c.created_at,c.id,i.display_order,i.created_at,i.id`,
      [restaurantId, availableOnly ? true : null],
    );
    return rows;
  }
}
