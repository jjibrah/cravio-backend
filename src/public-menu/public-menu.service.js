...
const unavailable = () =>
  new AppError(404, 'PUBLIC_MENU_NOT_FOUND', 'Public menu is not available');

export class PublicMenuService {
  constructor({ publicMenus, tables }) {
    this.publicMenus = publicMenus;
    this.tables = tables;
  }
  shapeCategories(rows) {
    ...
    for (const row of rows) {
      ...
      category.items.push({
        ...
        media:
          row.video_url || row.thumbnail_url
            ? {
                thumbnail_url: row.thumbnail_url || null,
                video_url: row.video_url || null,
                duration_seconds:
                  row.duration_seconds === undefined ? null : Number(row.duration_seconds),
              }
            : null,
      });
    }
  ...
  async getByQrToken(token, availableOnly = false) {
    let qr;
    try {
      qr = await this.tables.resolveQrToken(token);
    } catch (error) {
      if (error instanceof AppError && error.status === 404) throw unavailable();
      throw error;
    }
