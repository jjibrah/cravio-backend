...
import {
  createPublicMenuLimiter,
  createPublicMenuRouter,
} from './public-menu/public-menu.routes.js';
...
export function createApp(options = {}) {
  ...
  app.use(helmet());
  app.use(
    cors({ origins: appConfig.corsOrigins || [], production: appConfig.nodeEnv === 'production' }),
  );
  ...
  const mediaController = createMediaController(
    new MediaService({
      ...
      restaurants,
      storage,
      processor: mediaProcessor || createVideoProcessor(appConfig.media),
      limits: mediaLimits || appConfig.media,
      logger,
    }),
  );
  ...
  const tableService = new TableService({
    tables,
    restaurants,
    qr: qrService || createQrService(),
    publicAppUrl: publicAppUrl || appConfig.publicAppUrl,
  });
  ...
  const publicMenuController = createPublicMenuController(
    new PublicMenuService({ publicMenus, tables: tableService }),
  );
  ...
  const analyticsController = createAnalyticsController(
    new AnalyticsService({ analytics, tables: tableService, restaurants }),
  );
  ...
  app.use(
    ...
    createHealthRouter({
      db,
      storage: healthStorage || storage,
      timeoutMs: appConfig.healthTimeoutMs || 2000,
    }),
  );
  ...
  app.use(
    ...
    createPublicMenuRouter({
      ...
      limiter:
        publicMenuLimiter ||
        createPublicMenuLimiter({ limit: appConfig.rateLimits?.publicMenu || 120 }),
    }),
  );
  app.use(
    ...
    createPublicAnalyticsRouter({
      ...
      limits:
        analyticsLimiters ||
        createAnalyticsLimiters({
          sessionLimit: appConfig.rateLimits?.analyticsSessions || 30,
          eventLimit: appConfig.rateLimits?.analyticsEvents || 300,
        }),
    }),
  );
  ...
  app.use(
    '/api/admin',
    adminLimiter || createRateLimiter({ limit: appConfig.rateLimits?.admin || 120 }),
    createAdminRouter({ auth, controller: adminController }),
  );
  ...
  app.use(
    ...
    createMediaRouter({
      ...
      uploadLimiter:
        mediaUploadLimiter ||
        createMediaUploadLimiter({ limit: appConfig.rateLimits?.media || 20 }),
    }),
  );
