...
const price = z.union([z.string(), z.number()]).transform((value, ctx) => {
  ...
  if (!/^(?:0|[1-9]\d{0,9})(?:\.\d{1,2})?$/.test(text)) {
    ctx.addIssue({
      code: 'custom',
      message: 'Price must be a non-negative decimal with at most two decimal places',
    });
    return z.NEVER;
  }
...
export const reorderItemsSchema = z
  .object({
    ...
    item_ids: z
      .array(id)
      .min(1)
      .refine((ids) => new Set(ids).size === ids.length, 'Duplicate menu item IDs are not allowed'),
  })
  .strict();
