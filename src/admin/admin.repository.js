export class AdminRepository {
  ...
  async listRestaurants({ page, limit, status, published, search }) {
    ...
    if (published !== undefined) { values.push(published); where.push(`r.is_published=$${values.length}`); }
    if (search) { values.push(`%${search}%`); where.push(`(r.name ILIKE $${values.length} OR r.slug ILIKE $${values.length} OR u.email ILIKE $${values.length} OR concat_ws(' ',u.first_name,u.last_name) ILIKE $${values.length})`); }
    const clause = where.length ? `WHERE ${where.join(' AND ')}` : '';
    const offset = (page - 1) * limit; values.push(limit, offset); const limitAt = values.length - 1;
    const [items, count] = await Promise.all([
      this.db.query(`SELECT r.id,r.name,r.slug,r.status,r.is_published,r.created_at,
        ...
        ORDER BY r.created_at DESC,r.id LIMIT $${limitAt} OFFSET $${limitAt + 1}`, values),
      this.db.query(`SELECT COUNT(*)::int AS total FROM restaurants r JOIN users u ON u.id=r.owner_id ${clause}`, values.slice(0, limitAt - 1))
    ]);
    return { items: items.rows, total: count.rows[0].total };
  }
  async getRestaurant(id) {
    const { rows } = await this.db.query(`SELECT r.id,r.name,r.slug,r.description,r.logo_url,r.cover_image_url,r.phone,r.email,r.address,r.city,r.country,r.currency,r.status,r.is_published,r.created_at,r.updated_at,r.published_at,
      json_build_object('id',u.id,'email',u.email,'first_name',u.first_name,'last_name',u.last_name,'status',u.status) AS owner,
  ...
  async listOwners({ page, limit, status, search }) {
    const values = []; const where = ["u.role='owner'"];
    if (status) { values.push(status); where.push(`u.status=$${values.length}`); }
    if (search) { values.push(`%${search}%`); where.push(`(u.email ILIKE $${values.length} OR concat_ws(' ',u.first_name,u.last_name) ILIKE $${values.length} OR r.name ILIKE $${values.length})`); }
    const clause = `WHERE ${where.join(' AND ')}`; const offset = (page - 1) * limit; values.push(limit, offset); const limitAt = values.length - 1;
    const [items, count] = await Promise.all([
      this.db.query(`SELECT u.id,u.email,u.first_name,u.last_name,u.role,u.status,u.created_at,
        ...
        ORDER BY u.created_at DESC,u.id LIMIT $${limitAt} OFFSET $${limitAt + 1}`, values),
      this.db.query(`SELECT COUNT(*)::int AS total FROM users u LEFT JOIN restaurants r ON r.owner_id=u.id ${clause}`, values.slice(0, limitAt - 1))
    ]);
    return { items: items.rows, total: count.rows[0].total };
  }
  async getOwner(id) {
    const { rows } = await this.db.query(`SELECT u.id,u.email,u.first_name,u.last_name,u.role,u.status,u.created_at,u.updated_at,
      ...
      FROM users u LEFT JOIN restaurants r ON r.owner_id=u.id WHERE u.id=$1 AND u.role='owner'`, [id]);
    return rows[0] || null;
  }
  async countActiveAdmins() { const { rows } = await this.db.query("SELECT COUNT(*)::int AS total FROM users WHERE role='admin' AND status='active'"); return rows[0].total; }
  async metrics() {
  ...
  async changeWithAudit({ adminId, table, id, status, role, action, targetType }) {
    ...
    try {
      ...
      const current = await client.query(`SELECT id,status${table === 'users' ? ',role,email,first_name,last_name,created_at,updated_at,clerk_user_id' : ''} FROM ${table} WHERE id=$1 FOR UPDATE`, [id]);
      if (!current.rowCount) { await client.query('ROLLBACK'); return null; }
      const oldRow = current.rows[0]; const column = status !== undefined ? 'status' : 'role'; const value = status ?? role;
      const updated = await client.query(`UPDATE ${table} SET ${column}=$2,updated_at=NOW() WHERE id=$1 RETURNING *`, [id, value]);
      await client.query(`INSERT INTO admin_audit_logs (admin_user_id,action,target_type,target_id,old_values,new_values) VALUES ($1,$2,$3,$4,$5,$6)`, [adminId, action, targetType, id, { [column]: oldRow[column] }, { [column]: value }]);
