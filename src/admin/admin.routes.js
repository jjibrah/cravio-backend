...
export function createAdminRouter({ auth, controller }) {
  const router = Router();
  router.use(auth, requireActiveAccount, requireRole('admin'));
  router.get('/dashboard', validate(dashboardSchema, 'query'), asyncHandler(controller.dashboard));
  router.get(
    '/restaurants',
    validate(restaurantListSchema, 'query'),
    asyncHandler(controller.listRestaurants),
  );
  router.get(
    '/restaurants/:id',
  ...
  router.patch(
    ...
    asyncHandler(controller.setRestaurantStatus),
  );
  ...
  router.patch(
    ...
    validate(idParamsSchema, 'params'),
    validate(statusSchema),
    asyncHandler(controller.setOwnerStatus),
  );
