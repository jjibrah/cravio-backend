...
function identity(data) {
  const primary =
    data.email_addresses?.find((item) => item.id === data.primary_email_address_id) ||
    data.email_addresses?.[0];
  if (!primary?.email_address)
    throw new AppError(400, 'INVALID_WEBHOOK', 'Clerk user has no email address');
  return {
    clerkUserId: data.id,
...
export function createClerkWebhookHandler({ users, webhookVerifier = verifyWebhook }) {
  return async (req, res, next) => {
    try {
      let event;
      try {
        event = await webhookVerifier(req);
      } catch {
