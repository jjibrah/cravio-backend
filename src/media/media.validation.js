...
export const uploadSchema = z.object({ mime_type: z.enum(['video/mp4', 'video/webm']), original_filename: z.string().trim().min(1).max(255).refine((value) => !/[\\/]/.test(value), 'Filename must not contain a path').optional() }).strict();
