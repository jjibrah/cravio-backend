...
export const createMediaController = (service) => ({
  createVideoUpload: async (req, res) => ok(res, await service.createVideoUpload(req.auth.user.id, req.params.itemId, req.body), 201),
  completeVideo: async (req, res) => ok(res, await service.completeVideo(req.auth.user.id, req.params.id)),
