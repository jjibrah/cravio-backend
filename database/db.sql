...
CREATE TABLE IF NOT EXISTS media_assets (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT media_assets_item_restaurant_fk FOREIGN KEY (menu_item_id, restaurant_id) REFERENCES menu_items (id, restaurant_id) ON UPDATE CASCADE ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS restaurants_public_lookup_idx ON restaurants (slug)
WHERE
  status = 'active'
...
CREATE UNIQUE INDEX IF NOT EXISTS media_assets_one_ready_per_item_idx ON media_assets (menu_item_id)
WHERE
  status = 'ready';
...
CREATE INDEX IF NOT EXISTS analytics_events_item_created_idx ON analytics_events (menu_item_id, created_at)
WHERE
  menu_item_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS analytics_events_category_created_idx ON analytics_events (category_id, created_at)
WHERE
