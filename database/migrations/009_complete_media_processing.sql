...
ADD CONSTRAINT media_assets_duration_check CHECK (
  duration_seconds > 0
...
ADD CONSTRAINT media_assets_ready_fields_check CHECK (
  status <> 'ready'
  OR (
...
CREATE UNIQUE INDEX media_assets_one_ready_per_item_idx ON media_assets (menu_item_id)
WHERE
  status = 'ready';
